...
void edison_min_init_layer1(void)
{
 ...
 */
void edison_min_rx_byte(uint8_t byte)
{
	...
	switch(rx_frame_state) {
        ...
        case RECEIVING_EOF:
            if(byte == 0x55u) {
                ...
                edison_min_frame_received(rx_frame_buf, rx_control, rx_frame_id);
            }
            /* else discard */
            /* Look for next frame */
            rx_frame_state = SEARCHING_FOR_SOF;
 ...
 *
 */
void edison_min_tx_frame(uint8_t id, uint8_t payload[], uint8_t control)
{
	uint8_t n, i;
	uint16_t checksum;
